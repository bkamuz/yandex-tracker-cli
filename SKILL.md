...
Команды `attachment-download` и `attachment-upload` допускают только пути внутри разрешённой директории (по умолчанию — текущая рабочая директория). Это снижает риск чтения или записи произвольных файлов при использовании CLI (в т.ч. агентом).

- **YANDEX_TRACKER_ATTACHMENTS_DIR** — опциональная переменная окружения: базовая директория для вложений (например `~/yandex-tracker-files`). Если не задана, используется текущая директория.
- **YANDEX_TRACKER_ALLOW_ANY_PATH=1** — отключает проверку путей (не рекомендуется при использовании с AI-агентами).
